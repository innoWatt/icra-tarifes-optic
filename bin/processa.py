...
'''
    ...
    string de bytes, com per exemple: ('\x10\x49\x01\x00\x4a\x16')
...
def processa(missatge):
    '''
        Analitza la trama i crea un format llegible semblant a XML, com ara:
            <missatge>
                <control>
                        [...]
                </control>
    ...
    '''
    global buf #crea variable global pq altres funcions la puguin veure

    '''transforma a bytearray'''
...
def campObjsInfo(objsInfo):
    '''
        ...
        estructura (bytes):
            ...
            1 byte        variable        5 ò 7 bytes (tipus a ò b)
        +----------+---------------+-------------------+
    ...
    '''Si saps l'ASDU, perfecte, sinó, intenta endevinar-ho'''
    if(idt in [8,11]):
...
def campObjInfo(objInfo):
    ...
    if(idt in [8,11]):
        ...
        '''ASDU 162 PETICIO LLEGIR INSTANTANIS (la resposta es un ASDU 163)'''
        #1 byte
        print("        Direcció objecte: "+str(objInfo[0]))
    elif(idt in [163]):
        ...
        ''' A187: petició FINALIZAR SESIÓN
            ...
            in_lo_lo_in_co_di_di_[   ASDU==IUD   ]_cs_fi
            és un objecte d'informació buit
        '''
        print("        Request FINALITZAR SESSIÓ")
    else:
        print("        ASDU "+str(idt)+" ENCARA NO IMPLEMENTAT")
...
def campTotalIntegrat(totalIntegrat,nom):
    ...
    nrg_valor = nrg[3] << 24 | nrg[2] << 16 | nrg[1] << 8 | nrg[0]
...
'''peticio asdu 162'''
#processa("\x68\x0a\x0a\x68\x73\x01\x00\xa2\x01\x05\x01\x00\x00\xc0\xdd\x16")
